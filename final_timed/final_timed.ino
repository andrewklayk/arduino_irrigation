...
void loop() {
  ...
  for (int i = 0; i < nCells; i++)
  {
    ...
    if (Cells[i].timeRegulated == false)
    {
    ...
    {
      if(!Cells[i].valveIsOpen && isWateringTime())
      {
        digitalWrite(Cells[i].waterValve, HIGH);
        Cells[i].valveIsOpen = true;
        Serial.print("Opened  ");
...
bool isWateringTime() {
  return (TimeIsBetween(mHourOn, mMinuteOn, time.Hours, time.minutes, mHourOff, mMinuteOff) || TimeIsBetween(eHourOn, eMinuteOn, time.Hours, time.minutes, eHourOff, eMinuteOff))
}
